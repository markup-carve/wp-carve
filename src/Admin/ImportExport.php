...
class ImportExport
{
    ...
    public function register(): void
    {
        ...
        add_action('admin_post_wp_carve_import', [$this, 'handleImport']);
        add_action('admin_post_wp_carve_export', [$this, 'handleExport']);
    ...
    public function importPage(): void
    {
        ...
        wp_nonce_field('wp_carve_import');
        echo '<input type="hidden" name="action" value="wp_carve_import">';
        echo '<table class="form-table"><tbody>';
        echo '<tr><th><label for="wp_carve_file">' . esc_html__('File', 'carve-markup') . '</label></th>';
        echo '<td><input type="file" name="wp_carve_file" id="wp_carve_file" accept=".md,.markdown,.djot,.dj,.carve,.crv,.html,.htm,.txt" required></td></tr>';
        echo '<tr><th><label for="wp_carve_title">' . esc_html__('Title', 'carve-markup') . '</label></th>';
        echo '<td><input type="text" name="wp_carve_title" id="wp_carve_title" class="regular-text" placeholder="' . esc_attr__('Defaults to the file name', 'carve-markup') . '"></td></tr>';
        echo '</tbody></table>';
    ...
    {
        check_admin_referer('wp_carve_import');
        if (!current_user_can('edit_posts')) {
        ...
        update_post_meta((int)$postId, '_wp_carve_enabled', 1);
    ...
    public function rowAction(array $actions, WP_Post $post): array
    {
        ...
        $url = wp_nonce_url(
            admin_url('admin-post.php?action=wp_carve_export&post=' . $post->ID),
            'wp_carve_export_' . $post->ID,
        );
        $actions['wp_carve_export'] = '<a href="' . esc_url($url) . '">' . esc_html__('Export Carve', 'carve-markup') . '</a>';
    ...
    public function handleExport(): void
    {
        ...
        check_admin_referer('wp_carve_export_' . $id);
